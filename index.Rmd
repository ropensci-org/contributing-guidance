...
Read [rOpenSci & Our Community](#intro) learn about our mission, what it means to be part of our community, and to meet some [Humans of rOpenSci](#humans).
...
# Get number of package authors

# Get rOpenSci packages
pkgs <- "https://ropensci.github.io/roregistry/registry.json" %>%
  jsonlite::fromJSON(.) %>%
  .[["packages"]] %>%
  dplyr::filter(stringr::str_detect(status, "active"))

pkgs %>%
  dplyr::filter(stringr::str_detect(url, "ropensci") | onboarding != "")

nrow(pkgs)
dplyr::pull(pkgs, maintainer) %>% unique() %>% length()
